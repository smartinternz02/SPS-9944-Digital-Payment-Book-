...
def register():
    ...
    if request.method == 'POST':
        ...
        if account:
            ...
            cursor.execute('INSERT INTO users(name,email,password) VALUES(% s,% s,% s)',(username,email,hash_password))
...
def dashboard():
    if 'loggedin' not in session.keys():
        ...
        if 'loggedin' in session.keys() and session['isretailer'] == 0:
            ...
            cursor.execute("SELECT name,email FROM users WHERE userid = % s",[session['id']])
